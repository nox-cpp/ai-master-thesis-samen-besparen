...
# TODO: this experiment setup is almost a duplicate of that of the importance
# space experiment, factor this into functions or a template file
...
# load the most recent imputed data set as our basis
population_basis <- get_newest_data_file_with_base_path(
  file.path(base_data_dir), "combined_data", "imputed"
)

# check if the input data is valid
if (is.na(population_basis)) {
  stop(paste0(
    "Input data is not available, please run the data collection ",
    "and data processing scripts prior to running this experiment setup."
  ))
}

# load the default dataset
population_base <- load_humat_input_set(population_basis)

# TODO: generate a random population basis as well as a sanity check
...
# NOTE: this conversion will be the same as that of the `baseline` config,
# but the config is hard-coded into the function. The default_humats and
# `baseline' may converge if either changes. In any case, this is not intended
# for experiments, but for development purposes
default_humats <- humat_convert_data_to_agents(population_base)
...
for (path in c(latest_population_output_dir)) {
  ...
  default_output_path <- file.path(path, "script_default.csv")
...
if (REGENERATE_MAPPINGS) {
  # add the mappings for select configurations, stored in files
  mapping_cfg <- mapping_cfg |>
    bind_rows(read.csv(file.path(
      mapping_cfg_folder, "pessimistic_financials.csv"
    )))
  mapping_cfg <- mapping_cfg |>
    bind_rows(read.csv(file.path(mapping_cfg_folder, "value.csv")))
  mapping_cfg <- mapping_cfg |>
    bind_rows(read.csv(file.path(mapping_cfg_folder, "comfort.csv")))
} else {
...
mapping_cfg_output <- file.path(
  mapping_cfg_folder, generate_file_name(experiment_name, suffix = "cfg")
)
...
for (row in rownames(mapping_cfg)) {
  ...
  humat_input_set <- population_base
