...
source(file.path(script_dir, "experiments", experiment_name, "helpers.r"))
...
print(
  ...
  file = file.path(table_dir, "cors_sim_preds_batch_one.tex")
)
...
print(
  ...
  include.rownames = FALSE,
  file = file.path(table_dir, "joint_sim_pred_table_batch_one.tex")
)
...
print(
  ...
  include.rownames = FALSE,
  file = file.path(table_dir, "joint_sim_pred_table_convs_batch_one.tex")
)
...
print_labeled_summary(
  bn_summary,
  dir = table_dir,
  file_name_prefix = "bayesian_batch_one",
  sub_name = "bayesian-one"
)
...
print_labeled_summary(
  ipf_summary,
  dir = table_dir,
  file_name_prefix = "ipf_batch_one",
  sub_name = "ipf-one"
)
...
# agent choice
ggsave(
  filename = file.path(figure_dir, "batch_one_choice_plots.pdf"),
  annotate_figure(
    ggarrange(
...
# communication
ggsave(
  filename = file.path(figure_dir, "batch_one_comm_plots.pdf"),
  annotate_figure(
    ggarrange(
...
ggsave(
  filename = file.path(figure_dir, "batch_one_group_comm_plots.pdf"),
  annotate_figure(
    ggarrange(
...
# satisfaction
ggsave(
  filename = file.path(figure_dir, "batch_one_satisfaction_plots.pdf"),
  annotate_figure(
    ggarrange(
...
test_plot <- test_data |> ggplot(aes(x = condition, y = value_mean, fill = name)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin = value_mean - value_sd, ymax = value_mean + value_sd), position = "dodge")
